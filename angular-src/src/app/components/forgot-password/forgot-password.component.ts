    ...
    if(this.username == undefined || this.username ==''){
      ...
      this.authService.sendVerificationLink(user).subscribe(data => {
        ...
        if(data.success){
          ...
          localStorage.setItem('reset_password_token', data.user.reset_password_token);
          localStorage.setItem('reset_password_expires', data.user.reset_password_expires);

          this.router.navigate(['/forgotConfirmation']);          
        } else {
          this.flashMessagesService.show(data.msg.response, {
            cssClass: 'alert-danger',
            timeout: 7000
          });
        }
